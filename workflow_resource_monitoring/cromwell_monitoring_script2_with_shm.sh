...
#
# Source: This script was copied from the following location and enhanced to include monitoring of shared memory:
# # https://github.com/talkowski-lab/gatk-sv-internal/blob/main/scripts/cromwell/cromwell_monitoring_script2.sh
# Documentation for using it Terra is available here:
# Monitoring GCP cloud resources used in a workflow
# https://support.terra.bio/hc/en-us/articles/27341964316699-Monitoring-GCP-cloud-resources-used-in-a-workflow
set -Eeuo pipefail
...
function getShmem() {
    # get shared memory in use from /proc/meminfo (Shmem field),
    # reported in GiB and as a percentage of /dev/shm total capacity
    SHM_TOTAL_KB=$(df /dev/shm 2>/dev/null | awk 'NR==2 {print $2}')
    awk -v shm_total="${SHM_TOTAL_KB:-0}" '/^Shmem:/ {
        used = $2
        printf "%.2f\t", used / 1048576
        if (shm_total > 0) {
            printf "%.1f", 100 * used / shm_total
        } else {
            printf "nan"
        }
    }' /proc/meminfo
...
echo Total Shared Memory: $(getDisk Size /dev/shm) GiB
echo Total Disk space: $(getDisk Size "$MONITOR_MOUNT_POINT") GiB
echo Start time: $(date "+%F %T %z" -d @$T_START)
echo --- Runtime Information ---
echo -e "ElapsedTime\tCPU\tMem\tMemPct\tShMem\tShMemPct\tDisk\tDiskPct\tIORead\tIOWrite"
echo -e "HH:MM:SS   \t%\tGiB\t%\tGiB\t%\tGiB\t%\tMiB/s\tMiB/s"
